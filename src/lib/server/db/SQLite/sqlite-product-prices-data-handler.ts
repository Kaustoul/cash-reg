...
export const sqliteProductPrices: ProductPricesDataHandler = {
    ...
    async fetchPrice(db: BetterSQLite3Database | SQLiteTx, priceId: number): Promise<IPrice | null> {
        const res = await db
            .select()
            .from(productPricesTable)
    ...
    async newPrice(db: BetterSQLite3Database | SQLiteTx, price: Omit<IPrice, "priceId">): Promise<number> {
        const res = await db
