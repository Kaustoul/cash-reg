...
export function updateItemQuantity(cart: IShoppingCart, item: IShoppingCartItem, newQuantity: Decimal): void {
    item.quantity = newQuantity;
    calculateItemTotal(item);
    calculateCartTotal(cart);

    console.log(`Item ${item.name} quantity updated to ${item.quantity}`);
}
